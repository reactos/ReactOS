...
/* Always perform 32-bit I/O access */
#define ATA_ALWAYS_DO_32_BIT_IO
...
/* Delay of 400ns */
#if defined(SARCH_PC98)
    /* Write any value to 0x5F port to stall the bus */
    #if defined(_MSC_VER)
    #define ATA_IO_WAIT()    __asm { out 0x5F, al }
    #elif defined(__GNUC__)
    #define ATA_IO_WAIT()    __asm__ __volatile__("outb %%al,%0" : : "N" (0x5f))
    #else
    ...
    _In_ ULONG SectorCount)
{
    ...
    return (((SectorNumber + SectorCount) >= 0x0FFFFFFFULL) || (SectorCount > 0x100));
}
