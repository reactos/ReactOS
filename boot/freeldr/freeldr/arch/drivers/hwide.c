    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    for (Retry = 0; Retry < 3; ++Retry)
    {
        ...
        {
            ERR("ATA%s command %02X failed %u %02X at %X:%u\n",
    ...
    _In_ ULONG SectorCount)
{
    ...
    else
    {
        ...
        TaskFile->Data[0].SectorCount = (UCHAR)SectorCount;
        TaskFile->Data[0].LowLba = (UCHAR)Cylinder;
        TaskFile->Data[0].MidLba = (UCHAR)Sector;
        TaskFile->Data[0].HighLba = (UCHAR)(Sector >> 8);
    ...
    _In_ PHW_DEVICE_UNIT DeviceUnit)
{
    ...
    if (!AtapWaitForNotBusy(Registers, ATA_TIME_BUSY_ENUM, NULL))
    {
        if (!AtapPerformSoftwareReset(DeviceUnit))
            return FALSE;
    }
    ...
    _In_ PHW_DEVICE_UNIT DeviceUnit)
{
    ...
    /* Check for ATA */
    if (AtapReadIdenifyData(DeviceUnit, IDE_COMMAND_IDENTIFY))
        return DEV_ATA;

    /* Check for ATAPI */
    if (AtapReadIdenifyData(DeviceUnit, IDE_COMMAND_ATAPI_IDENTIFY))
        return DEV_ATAPI;
...
AtapInitDevice(
    ...
    _In_ ATA_DEVICE_CLASS DeviceClass)
{
    if (!AtapAnalyzeIdenifyData(DeviceUnit, DeviceClass))
        return FALSE;
